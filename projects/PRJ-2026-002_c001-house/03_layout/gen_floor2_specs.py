...
construction. The ONLY hand-authored inputs per loose piece are:
    ...
    facing    — cardinal rotation (a bbox has no front; read from the symbol / owner)
...
merged pieces?) and facing remain HUMAN calls, surfaced in the REVIEW checklist. This generator
adds a curve/shape sanity check + snap-distance guard so an anchor cannot quietly grab the wrong
cluster, but it does not replace the human identity sign-off.
...
except ImportError:                       # allow running from repo root or the layout dir
    import os
...
def snap(room, name, kind, clusters, claimed, anchor, face, h, **extra):
...
# flagged for the human. 7'x6.5' -> E-W(head-foot) 1981, N-S(width) 2134.
master_items.append(to_spec("เตียง 7'x6.5' หัวตะวันออก", "bed", 4115, 1090, 1981, 2134,
                            270, 600, note="head EAST vs BF14 slat; foot faces the west TV; "
...
master_items.append(snap("master_bedroom", "โต๊ะข้างเตียง เหนือ", "side_table", mc, m_claimed,
                         (4944, 2456), "S", 520, note="NE corner of the bed head"))
master_items.append(snap("master_bedroom", "โต๊ะข้างเตียง ใต้", "side_table", mc, m_claimed,
                         (4944, -196), "N", 520, note="SE wall niche south of the bed head"))
...
master_items.append(snap("master_bedroom", "เก้าอี้ทำงาน (หันเข้าโต๊ะ W)", "armchair", mc, m_claimed,
                         (1125, 4253), "W", 750, note="pulled up to the BF11 work desk; faces W"))
...
master_items.append(snap("master_bedroom", "ตู้/ชั้นวางทีวี ผนังตะวันตก", "tv_console", mc, m_claimed,
                         (606, 1151), "S", 700, note="TV shelf on the west wall the bed foot faces"))
...
sit_items.append(snap("sitting_room", "โซฟา 3 ที่นั่ง", "sofa", sc, s_claimed,
                      (8164, 4652), "E", 800, note="faces EAST toward the round accent table"))
...
sit_items.append(snap("sitting_room", "เก้าอี้ tub ซ้าย (หันออกสู่ระเบียง)", "armchair", sc, s_claimed,
                      (6331, 950), "S", 750, note="faces the terrace planters (south)"))
sit_items.append(snap("sitting_room", "เก้าอี้ tub ขวา (หันออกสู่ระเบียง)", "armchair", sc, s_claimed,
                      (7633, 1403), "S", 750, note="faces the terrace planters (south)"))
