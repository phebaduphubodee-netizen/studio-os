...
LAYOUT = os.path.join(REPO, "projects", "PRJ-2026-002_c001-house", "03_layout")
OUT = os.path.join(REPO, "pipeline", "output", "floor2", "floor2_keyplan.png")
...
manifest = load("floor2-manifest.json")
...
ax.set_title("FLOOR 2 KEY PLAN — footprint ทาบบนแบบจริง (เลข = ดู legend ขวา). "
             "ขนาด/ทิศ อ่านจาก scene-graph (mm); \"7'x6.5'\" = ป้ายเตียงในแบบเดิม", fontsize=12)
...
foot = [
    ...
    f"REVIEW ← identity human-confirm ({_hc})   ·   floating 0 + unplaced 0 = completeness (gate การันตี)",
    "facing_flags 0 = ไม่มี facing 'ขัดกับ ink' (ไม่ใช่ 'ยืนยันครบ'): เก้าอี้ tub ไม่มี rot → หัน S จาก note (ลูกศรจาง)",
    "ภาพ=สื่อสาร; หลักฐาน = พิกัด JSON + marker (per-piece IoU หลัง snap = tautological จึงไม่พิมพ์  ·  "
    "cluster# = room-scoped  ·  [!] = ป้าย≠วาด ตรวจ)",
]
