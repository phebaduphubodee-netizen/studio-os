...
## Colour legend (this bundle only)

These crops are a vector replay of the same page with ink coloured BY PEN
WEIGHT: **black** = heaviest pen (structural walls), **blue** = medium pens
(furniture, fixtures, and label text), **red** = thinnest pen (dimension lines,
arrows, extension lines, hatching, small annotations). Same geometry, same
scale, same crop windows as the uncoloured original.
