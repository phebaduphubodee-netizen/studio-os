...
## 2026-08-17e — p2r51: the whole-bed audition, and what "the scope was the defect" bought

The cloth bench asked "which cloth covers OUR mattress", so 11 complete beds sat in
the cache unauditionable by construction. One session after that sentence was
written, an audition instrument exists (wholebed_rules/bench, 20 tests) and the
measured result is sharp: staged in the real room, a bought whole bed produces the
first frames in this lane's 51 rounds with NO bare mattress flank — the defect
leaves the frame by construction instead of being covered by drape.

Three learnings worth keeping beyond the round:

1. **A prefix is not a roster.** The previous session's "11 whole-bed folders"
   counted `bd_*` (the Buddha statue) and `th_*` (Thai altar pieces) as beds. The
   real roster came from opening every file (stdlib GLB inspector, then a
   27-candidate contact sheet LOOKED at frame by frame). Same law as R11, applied
   to one's own inventory.

2. **The acceptance band transfers.** The flank panel's construction (blind
   readers, delivered band, unlabeled control) reran on audition quicks and held —
   the p2r49 control read 'most' 3/3, delivered-band candidates separated cleanly.
   A criterion read off delivered work once can price every future purchase; that
   is what "the acquisition now has an acceptance test" means in practice.

3. **The instrument corrected its author three times in one day** — pillow banks
   deleted as headboards, nightstand wings tagged as headboards, and a "negative
   control" footboard that was measured RIGHT (it tops out below its own sleeping
   plane) where the author's prior was wrong. Each correction is now a test. An
   audition that cannot surprise its author is a mirror, not an instrument.

Also recorded: a first draft of the gate's panel table was written before reader B
returned, with invented B values — caught and replaced with measured ones before
