...
def reconcile(rows, dump, frame_name, today=None):
    ...
    n_match = n_gap = n_unres = n_block = 0
    for row in rows:
    ...
    return rows, {"drawn": len(rows), "matched": n_match, "gaps": n_gap,
                  "unresolved": n_unres, "blocking": n_block,
...
def ledger_summary(ledger):
    ...
    s = {"drawn": len(rows), "matched": 0, "gaps": 0, "unresolved": 0,
         "blocking": 0, "frustum_source": "stored"}
    frame = None
    warns = []
    for r in rows:
        v = r.get("verdict")
