...
def count_items(records):
    ...
    flatter it. Returns (n_items, sorted ids, ran); ran is False when no
    record carries `in_frustum` — an old dump must read NOT RUN, never 0
    (a 0 from a reader that cannot see is not a 0)."""
    ...
    for r in records:
        if (r.get("hidden_render") or not r.get("in_frustum")
                or r.get("occluded")):
            continue
        i = item_id(r.get("name", ""))
        if i:
            ids.add(i)
    return len(ids), sorted(ids), True
